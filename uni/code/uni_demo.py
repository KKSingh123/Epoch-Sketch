...
uni_demo.py
===========
Compare 8 sketches on the UNIX/UNI dataset (univ1_pt0.csv).

CSV columns:
    frame.number, frame.time_relative, eth.src, eth.dst,
    ip.src, tcp.srcport, ip.dst, tcp.dstport,
    frame.len, ip.len, ip.proto, tcp.analysis.ack_rtt

Flow key: 5-tuple (ip.src, tcp.srcport, ip.dst, tcp.dstport, ip.proto)
encoded as a canonical UTF-8 byte string.
...
Usage
-----
    python uni_demo.py --csv dataset/UNI/univ1_pt0.csv 
"""
...
def load_trace(path: str, limit: int | None = None) -> list[bytes]:
    """
    Read the UNI CSV; return list of flow-key byte strings.
    Flow key = 'srcIP:srcPort->dstIP:dstPort/proto'
    Rows with missing ip.src or ip.dst are skipped.
    """
    packets: list[bytes] = []
    with open(path, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if not src_ip or not dst_ip:
                continue  # malformed / non-IP row

            key = f"{src_ip}:{src_port}->{dst_ip}:{dst_port}/{proto}".encode()
            packets.append(key)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="8-sketch comparison on UNI trace.")
    ap.add_argument("--csv",     default="dataset/UNI/univ1_pt0.csv",
                                 help="Path to the UNI CSV file")
...
def _run(args) -> None:
    ...
    countless = CountLessSketch(num_rows=R, row_width=W_base)
    ...
    # ------------------------------------------------------------------
    print(
        f"\nRun Config:  data={args.csv}  sample={'all' if not args.sample else args.sample}"
