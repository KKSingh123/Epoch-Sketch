...
====================
Run all sketches on CAIDA trace with varying memory budgets (16KB to 256KB)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Sketch comparison memory experiment on CAIDA trace.")
    ap.add_argument("--dat",     default="dataset/CAIDA/0.dat",  help="Path to .dat trace file")
...
def _run(args) -> None:
    ...
    filenames = ['throughput.png', 'are_top20.png', 'are_all.png', 'are_hh.png']
    ...
    for metric, ylabel, filename in zip(metrics, ylabels, filenames):
        ...
        for name in results:
            ...
            ax.plot(x_positions, results[name][metric], marker='o', label=label)
